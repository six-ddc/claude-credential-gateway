...
//   - 「哪些 Claude 相关主机经网关」也一样,改宽了就等于把网关变成通用出口代理。
//
// 设备侧分流代理(device.go)默认把 api.anthropic.com 加上这份名单送来网关,名单外的主机
// (WebFetch 抓的站点、第三方 MCP server、包源)由设备本机直连,不到网关。两边共用这一份
// 名单,所以要放行别的 Claude 相关主机就改这里、重新编译,让它进代码评审。
//
