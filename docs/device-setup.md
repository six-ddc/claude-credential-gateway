...
正常情况下这一节你不需要看：`setup-device.sh` 已经在 `~/.ccgw/claude-home/.claude.json` 里种好了
`hasCompletedOnboarding` 和 `theme`，全新机器第一次跑 `ccgw` 会直接进主界面，标题栏显示
`Claude Max`，**不会让你去浏览器登录**。第一次进某个目录时只有一个「是否信任此文件夹」的确认，
回车即可（下面「各记各的」那段解释了为什么真 `claude` 信任过的目录这里还要再确认一次）。

**万一 `ccgw` 起来后还是让你去登录账号**（比如用的是旧版包装命令，或那个文件被清掉了），
就是撞上了 Claude Code 的首次运行引导——它检测到没完成 onboarding 就会走引导流程，
**这一步不会因为设了 token 就跳过**。手工补的办法在下面。
